...
public class NestedCommitRecord {
    ...
    private final Cons<VArrayEntry<?>> varrayReadsToPropagate;	// TODO Propagate them
	...
	// TODO: PerTxBoxes lock-free support is currently under development in another branch
	// In that branch, there is also support for PerTxBoxes taking into account parallel 
	// nested transactions. Once it is guaranteed to be ok, it should be merged with 
	// this one (or with the lock-free branch if this made it there first).
...
//	}
	...
	if (this.arrayWritesToPropagate == ReadWriteTransaction.EMPTY_MAP) {
	...
	// nmld: I believe that this does not suffice. These writes are still racy with 
	// the read-after-writes that fetch values from ancestors, as both accesses (by 
	// committing nested transactions and executing nested transactions) will be 
	// manipulating a non-thread-safe hash map. In particular, the only problem 
	// that I envision is regarding the resize operation of the hash map. I could 
	// not reproduce those issues, and thus did not change the map that holds the 
	// VArray writes to be a thread-safe hash map as that would make it more 
	// expensive in its normal use in top-level transactions as well. If one day 
	// anyone finds this to be a problem, by using VArrays in parallel nested 
	// transactions, then this comment will surely be helpful.
	synchronized (parent) {
