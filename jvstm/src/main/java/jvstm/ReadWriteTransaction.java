...
public abstract class ReadWriteTransaction extends Transaction {
    ...
    /**
     * Validates this read-set against all active transaction records more
     * recent that the one <code>lastChecked</code>.
     * 
	...
	if (!this.bodiesRead.isEmpty()) {
		...
		if (array[i].body.version > myNumber) {
		    throw new CommitException(array[i].body.version);
		}
	    }
		...
		for (int i = 0; i < ar.length; i++) {
			...
			throw new CommitException(ar[i].body.version);
		    }
		}
	    }
	}
