...
import jvstm.scheduler.DefaultSchedulerFactory;
import jvstm.scheduler.Scheduler;
import jvstm.scheduler.SchedulerFactory;
import jvstm.scheduler.SchedulerWorker;
import jvstm.scheduler.SerializerScheduler;
import jvstm.scheduler.WaitRelaxedScheduler;
import jvstm.scheduler.WaitScheduler;
...
public abstract class Transaction {
    ...
    // This reference is maintained so that the scheduler may use the GC knowledge to map commit numbers to transactions
    public static GCTask gcTask = null;
    ...
    public static void initScheduling(Scheduler schedulerStrategy, int numberTransactionsIds) {
	schedulerStrategy.init(numberTransactionsIds);
    }

    public static void initScheduling(Scheduler schedulerStrategy, int numberTransactionsIds, int numberProcessors) {
	schedulerStrategy.init(numberTransactionsIds, numberProcessors);
    }

    /**
     * Used to create a read-write transaction that bypasses scheduling
     * @return
     */
    public static Transaction begin() {
	return begin(false, -1, null, null);
    }

    /**
     * Used to create a read-write transaction with scheduling support
     * @return
     */
    public static Transaction begin(int transactionId, Object work, SchedulerWorker runner) {
	return begin(false, transactionId, work, runner);
    }

    /**
     * Used to create a read-write/only transaction that bypasses scheduling
     * @return
     */
    public static Transaction begin(boolean readOnly) {
	return begin(readOnly, -1, null, null);
    }

    /**
     * Used to create a read-write/only transaction with scheduling support
     * @return
     */
    public static Transaction begin(boolean readOnly, int transactionId, Object work, SchedulerWorker runner) {
	...
	return beginWithActiveRecord(readOnly, activeRecord, transactionId, work, runner);
    }

    /**
     * Used internally by 'commitAndBegin'
     * @param readOnly
     * @param activeRecord
     * @return
     */
    protected static Transaction beginWithActiveRecord(boolean readOnly, ActiveTransactionsRecord activeRecord) {
	return beginWithActiveRecord(readOnly, activeRecord, -1, null, null);
    }

    // activeRecord may be null, iff the parent is also null, in which case activeRecord is not used, so it's ok!
    protected static Transaction beginWithActiveRecord(boolean readOnly, ActiveTransactionsRecord activeRecord,
	    int transactionId, Object work, SchedulerWorker runner) {
	Transaction parent = current.get();
	Transaction tx = null;
	...
	if (parent == null) {
		...
		tx = TRANSACTION_FACTORY.makeTopLevelTransaction(activeRecord, transactionId);
		tx.flattenNested = !(Scheduler.getScheduler().startTx(transactionId, work, runner));
	    }
	} else {
    ...
    public static void abort(int transactionId, int conflicterCommit) {
	Transaction tx = current.get();
	tx.abortTx();
	Scheduler.getScheduler().conflictTx(transactionId, conflicterCommit);
    }
