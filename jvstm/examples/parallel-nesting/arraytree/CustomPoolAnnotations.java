 ...
 * different branches.
 * This test uses the processors for both @Atomic and @ParNest.
 * @author nmld
 *
 */
public class CustomPoolAnnotations extends JVSTMTest<Long> {
    ...
    public CustomPoolAnnotations(String balance) {
	try {
	    Class<CustomPoolAnnotations> clazz = (Class<CustomPoolAnnotations>) this.getClass();
    ...
    public final int NUMBER_CONFLICTERS = 0;
	...
	public void execute() {
		...
		new ForkWork(index, incr, rootNode).exec();
	    } else {
	...
	public class ForkWork implements ParallelSpawn<Void> {

	    private final int baseId;
	    private final int idsToCover;
	    private final TreeLevel parentLevel;
	    
	    public ForkWork(int baseId, int idsToCover, TreeLevel parentLevel) {
		this.baseId = baseId;
	    ...
	    @Combiner
	    private Void combine(List<Void> arg) {
		return null;
	    }

	    @ParNest
	    private void parallelWork(int id, int idsToCover, TreeLevel level) {
		int SLICE = (ARRAY_SIZE / LEAFS);
		int base = id * SLICE;
		int top = (id + 1) * SLICE;
		...
		if (level.nextLevel == null) {
		    ...
		    int index = id;
		    new ForkWork(index, incr, level).exec();
		}
	    }
	    ...
	    public Void exec() {
		int tmp = baseId;
		for (int i = 0; i < parentLevel.numberChildren; i++) {
		    parallelWork(tmp, idsToCover, parentLevel.nextLevel);
		    tmp += idsToCover;
		}
	...
	Transaction.initThreadPool(128);
	new CustomPoolAnnotations(args[0]).test();
    }
